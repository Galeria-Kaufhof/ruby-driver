...
module Cassandra
  ...
  module Util extend self
    ...
    # BAD BAD BAD - this should fail
    def assert_nil_or_numeric_greater_zero(value, name = 'unknown')
      return true if value.nil?
      Util.assert_instance_of(::Numeric, value) { ":#{name} must be a number of seconds, #{value} given" }
      Util.assert(value > 0) { ":#{name} must be greater than 0, #{value} given" }
    end
